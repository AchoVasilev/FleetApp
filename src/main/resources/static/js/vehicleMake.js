...
function onDeleteModal(ev, deleteModal) {
    ...
    const vehicleMakeId = btn.id;

    const deleteBtn = document.getElementById('delete-btn');
    deleteBtn.addEventListener('click', ev => onDelete(ev, deleteModal, vehicleMakeId));
}

async function onDelete(ev, deleteModal, vehicleMakeId) {
    ev.preventDefault();

    try {
        const response = await fetch('/api/vehicleMake/' + vehicleMakeId, {
            method: 'delete',
            headers: {
...
async function onEditModal(ev, modalEl) {
    ...
    try {
        ...
        form.addEventListener('submit', e => handleSubmit(e, modalEl, vehicleMakeId))
    } catch (error) {
        console.log(error.message);
    }
}

async function handleSubmit(e, modalEl, vehicleMakeId) {
    e.preventDefault();
    ...
    try {
        const response = await fetch('/api/vehicleMake/editById/' + vehicleMakeId, {
            method: 'Put',
            headers,
